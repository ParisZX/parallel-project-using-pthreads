...
int main() {
	int done=0,j; long i; pthread_t threads[NUM_OF_THREADS]; int threadId[NUM_OF_THREADS];
...
void *map(void *arg) {
	...
	for(i=start+threadId;i<finish;i+=NUM_OF_THREADS) {
		...
		for(j=0;j<8;j++) {
			...
			if( (B[i][0]>=box[childsId]->limits[0][0])&&(B[i][0]<box[childsId]->limits[0][1])&&
				...
				pointsOfChild[j][box[childsId]->n-1][0]=B[i][0];
				pointsOfChild[j][box[childsId]->n-1][1]=B[i][1];
				pointsOfChild[j][box[childsId]->n-1][2]=B[i][2];
				unlock();				
				break;	
			}
		}
