...
int main() {
	...
	while(!done) {
		...
		#pragma omp parallel for shared(parents,newParents) private(i)
...
void map() {
	...
	#pragma omp parallel for private(j,i,childsId)
	for(j=0;j<8;j++) {	
...
void reduce() {
	...
	//#pragma omp parallel for shared(bCounter,numOfNewParents,newParents)
	for(i=0;i<8;i++) {
